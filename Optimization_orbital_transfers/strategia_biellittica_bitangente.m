...
[DeltaV_sp, om_f_sp, th_sp] = changeOrbitalPlane(a_circ, e_circ, i_i, OM_i, om_i, i_f, OM_f);  %trovo il punto in cui è possibile effettuare il cambio di piano, dal quale arto con la manovra biellittica bitangente (starting point)
...
i_T2 = i_f;
OM_T2 = OM_f;                                                               % dato che non so in che direzione sommare il DeltaV_cop per ottenere l'orientamento finale calcolo la traiettoria e poi aggiusto a posteriori OM, om e i
om_T2 = om_f_cop;                                                           % in ogni caso il risultato non cambia perche sommando il DeltaV_cop cambierei semplicemente l'orientazione del vettore velocità (e dell'orbita) ma non il suo modulo (--> considero l'orbita come se non la ruotassi e poi impongo io la rotazione cambiando i valori dei parametri solo per visualzzarla correttamente)

%% cambio anomalia del pericentro

[DeltaV_cpa, th_inizio_manovra, th_nuova_orbita] = changePericenterArg(a_f, e_f, om_f_cop, om_f);  

%% calcolo tempo attesa cambio pericentro

deltat_cpa = TOF(a_f, e_f, 0 , th_inizio_manovra(2));
...
DeltaV_totale = abs(DeltaV_cop) + abs(DeltaV_cpa) + abs(DeltaV1_bb) + abs(DeltaV2_bb) + abs(DeltaV1_circ) + abs(DeltaV2_circ) + abs(DeltaV3_bb);
Deltat_totale = Deltat_bb + deltat_circ + deltat_cpa + deltat_pf + deltat_sp;
